...
- ⚠️ COPY MISMATCH (open): the LP markets "100% local · audio never leaves your machine" but now serves the CLOUD variant (`Camie-Tech/inumaki`, posts audio to `/api/process`). The download fix was the user's explicit request; the privacy/comparison/FAQ copy + GitHub links now describe the cloud app. Decide: (a) update LP copy to match the cloud product, or (b) point the LP back at the local `inumaki-oss` build and release new local versions there. Flagged to user.
