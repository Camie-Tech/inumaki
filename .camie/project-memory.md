...
- RELEASE PROCESS for desktop updates: bump `apps/desktop/package.json` version → `pnpm --filter @inumaki/desktop exec vite build && pnpm --filter @inumaki/desktop exec electron-builder --win --publish never` → upload via `gh release create vX.Y.Z -R Camie-Tech/inumaki --target main` with assets `Inumaki-AI-Setup-X.Y.Z.exe`, `...Portable...`, `...Setup....exe.blockmap`, and **`latest.yml`** (latest.yml is REQUIRED for the updater; newest release must be GitHub "Latest"). Each release upload ~208MB; build the next version only AFTER the prior upload finishes (electron-builder overwrites `release/latest.yml`).
