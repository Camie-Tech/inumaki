...
- NOT done: runtime on-device verification (text actually landing in Notepad/VS Code/etc. via the global hotkey + mic capture) — REQUIRED before marketing the Wispr comparison per the roadmap exit criteria. Desktop binary NOT released (electron-builder + code signing is a NEXT-phase item). Commit `b9cda27` is local on `main`, unpushed.
