...
- VERIFIED: `pnpm --filter @inumaki/web typecheck` clean; `next build` exit 0 with `/sitemap.xml /robots.txt /manifest.webmanifest /opengraph-image /twitter-image` registered and `/` static (5m ISR). Dev-server curl confirmed all new sections + endpoints render and `/auth/signin`+`/dashboard` emit `noindex,nofollow`. NOT yet committed or deployed.
